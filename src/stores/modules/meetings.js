...
const actions = {
  ...
  async fetchEmployees({ commit }) {
    const response = await axios.get("?request=get-employees");
    commit("setEmployees", response.data);
  },
  async fetchOfficeHours({ commit }) {
    const response = await axios.get("?request=get-office-hours");
  ...
  async fetchCurrentMeetings({ commit }, params) {
    ...
    const response = await axios.get(
      `?request=get-current-meetings&timezone=UTC` + ids + start + end
    );
...
const mutations = {
  ...
  setCurrentMeetings: (state, current_meetings) =>
    (state.current_meetings = current_meetings),
  setSuggestionMeetings: (state, suggestion_meetings) =>
    (state.suggestion_meetings = suggestion_meetings),
